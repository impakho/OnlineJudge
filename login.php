...
// Check Register Form
if (isset($p_username) && strlen($p_username) > 0 && isset($p_password) && strlen($p_password) > 0 && isset($p_vcode) && strlen($p_vcode) > 0) {
    if (!isset($_SESSION['vcode']) || strtolower($p_vcode) !== $_SESSION['vcode']) {
        $_SESSION['message'] = '验证码错误！';
        $_SESSION['message_tag'] = 'error';
    }else if (!preg_match('/^[-_A-Za-z0-9]{1,16}$/', $p_username) || !preg_match('/^[\x20-\x7E]{1,32}$/', $p_password)) {
        ...
        $local_time = time();
        $result = RunDB("SELECT `id`, `password`, `logtime` FROM `user` WHERE `username` = '{$p_username}'");
        if (mysql_num_rows($result) <= 0) {
            ...
            if ($row['password'] !== $p_password) {
                ...
                RunDB("UPDATE `user` SET `logtime` = {$local_time} WHERE `id` = {$row['id']}");
                $_SESSION['user'] = $row['id'];
                $_SESSION['message'] = '登录成功！<div class="empty" style="width:25px;"></div>';
                $_SESSION['message'] = '上次登录时间：'.format_date_time_utc($row['logtime']).'（'.format_date($row['logtime']).'）';
...
<body>
    ...
    <div id="container">
    <div class="msg-info"><?php echo $GLOBALS['announce_login']; ?></div>
    <form action="" method="post">
        <input type="text" name="username" maxlength="16" placeholder="用户名" />
        <input type="password" name="password" maxlength="32" placeholder="密码" />
    ...
    <div class="empty" style="width:auto;height:50px;"></div>
    </div>
</body>
</html>
